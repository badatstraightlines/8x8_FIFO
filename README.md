...
1. Input data is written into the memory location addressed by the write pointer.
2. The write pointer advances to the next location.
3. FIFO status is updated accordingly.

### Read Operation

When `read_en` is asserted and the FIFO is not empty:

1. Data is read from the location addressed by the read pointer.
